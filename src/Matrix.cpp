...
template <typename T> void Matrix<T>::swapRows(size_t row1, size_t row2) {
...
template <typename T> void Matrix<T>::scaleRow(size_t row, T scalar) {
  if (row >= rows.size()) {
    throw std::out_of_range("Row index out of range");
  }
...
template <typename T>
Vector<T> Matrix<T>::forwardSubstitution(const Vector<T> &b,
...
template <typename T>
Vector<T> Matrix<T>::backwardSubstitution(const Vector<T> &y,
...
template <typename T> Matrix<T> Matrix<T>::rref() const {
  Matrix<T> result(*this);
  result.toRREF();
  return result;
}
